...
casesList  = [CHANNEL_POOL_SIZE_02, CHANNEL_POOL_SIZE_04,
              CHANNEL_POOL_SIZE_08, CHANNEL_POOL_SIZE_12, CHANNEL_POOL_SIZE_16]
...
class Gateway:
    ...
    def notify2Exec(self, cmdstr):
        print '===>Server Calling Remote Machine to Exec: ', cmdstr
        p=subprocess.Popen(cmdstr.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = p.communicate()
        return stderr
...
def experimentRun(snode):
    ...
    for currCase in casesList[:]:
        folderName = 'Task2Pool'+str(experiment_cases[currCase])+'/'
        ...
        for g in gatewaysList[:]:
            ...
            for node in nodesList:
                ...
                if int(currCase) >= CHANNEL_POOL_SIZE_02  and int(currCase) <= CHANNEL_POOL_SIZE_16:

                    fileCase = 'ExpChannels'+str(experiment_cases[currCase])+'n'+str(len(channel_cache[int(currCase)]))
