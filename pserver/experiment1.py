...
class CallbackThing(Pyro.core.ObjBase):
	  def __init__(self):                                
		  ...
		  #self.ch_pool_size = [12]
                  ...
                  self.multi_ch_cases= [CHANNEL_POOL_SIZE_CH15, CHANNEL_POOL_SIZE_CH20, CHANNEL_POOL_SIZE_CH1520,
	  ...
	  def save_data_to_file(self, data):
		  ...
		  if got_filename == None:
		      ...
		      if idx != -1:			  
                          filename = './results/'+exp_folder+exp_case+str('link'+sender_node_id+'-'+recv_node_id+'.txt')
			  got_filename = True
	  ...
	  def testfunc(self):
		  ...
		  channelcase=str('CH')+str(add_str)
                  return
              if int(addr_str) >= CHANNEL_POOL_SIZE_CH15  and int(str) <= CHANNEL_POOL_SIZE_CH15202526:
                  channelcase=''
		  #channelcase = experiment_cases[int(addr_str)]
                  channelcase = str('Pool')+str(len(channel_cache[int(add_str)]))
                  return 
              else:
	  ...
	  def mainloop(self):
	      global abort, channelcase, sender_nod_id, exp_case, exp_folder
	      ...
	      while abort != 1 and len(self.clients[:]) != 4:
		  try:
		      time.sleep(1)
		      n = int(n+1)
	      ...
	      ######We loop for each case
              ...
              #for gw in gateways:
              for sequence in range(68, 100):

                     node_id='' #reset the node_id..                    
                     found_node_id = False

                     exp_folder=''
                     if sequence < 10:
                        exp_folder = 'Folder0'+str(sequence)+'/'
                     else:
                        exp_folder = 'Folder'+str(sequence)+'/'

                     self.create_folder(exp_folder)

                     gw=[] #The gateway that the sender node is attached to.
                     for g in gateways:
                         if g.get_hostname().find('supernode1') != -1:
                             gw = g  #we have found the correct gateway.
			     print 'using: ', gw.get_hostname()
                             break;

                     moteList = [] #EXTRACT the list of motes on that cluster...
                     moteList = gw.get_cache()
                     for dev_idx, id_mote in moteList.iteritems():
                         if target_node == str(id_mote):
               
                            dev_id  = dev_idx
                            node_id = id_mote
                            print 'we have found the node: ',node_id,'@',g.get_hostname()
                            found_node_id = True
                            break

                     if node_id and found_node_id == True:
                           ...
                           for case_i in self.multi_ch_cases[:]:
                                ...
                                if int(case_i) >= CHANNEL_POOL_SIZE_CH15  and int(case_i) <= CHANNEL_POOL_SIZE_CH15202526:                                    
                                    exp_case = str('SEQ')
                                    if sequence < 10:
                                       exp_case = exp_case+'0'+str(sequence)+'Echannels'
                                    else:
                                        exp_case = exp_case+str(sequence)+'Echannels'


                                    exp_case = exp_case+str(experiment_cases[case_i])+'n'+str(len(channel_cache[int(case_i)]))
                                ...
                                devid = gw.get_devid_by_motename('KHZZ')
                                mote_id = int(devid) + 1
				      ...
				      time.sleep(0.5)
                                ...
                                print '==>trigger tx on node_id=',node_id, '.usb',devid
