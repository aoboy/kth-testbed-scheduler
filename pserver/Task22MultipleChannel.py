...
class NodeDownloadThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            endT = time.time()
            print '===>Node:',g.supernode,' download done! in ', (endT-startT)
...
def experimentRun(snode):
    ...
    #gatewaysList.append(Gateway('192.168.200.13', casesList[:1]))
    ...
    for currCase in casesList[:]:
        folderName = 'Task22/Tsch'+str(len(channel_cache[int(currCase)]))+'/'
