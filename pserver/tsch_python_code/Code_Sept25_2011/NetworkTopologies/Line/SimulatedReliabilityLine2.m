function rel_line = SimulatedReliabilityLine2(no_links, deadline, no_tests, link_trace)
...
for test = 1:no_tests,
    ...
    while ~done_test,
        ...
        start_trace = floor(1 + (length(link_trace.link(link).RxVec)-deadline-1).*rand); 
        index       = start_trace-1;
        done_link   = 0;
        while ~done_link,
            % successfull transmission
            index = index +1;
            if link_trace.link(link).RxVec(index) == 1,        % In case of success exit from while loop (done_link = 1) without updating 
            ...
            if link_trace.link(link).RxVec(index) == 0,         % if the packet is lost and there are transmission attempts left,
