...
int main(int argc, char** argv){
    ...
    char *device = devices[n_dev-1];

    signal(SIGINT, signal_handler);

    printf("===========================================================\n");
    printf("called by a remote app[cmd ./node_transmit %s]\n", device);
    ...
    if (fd < 0) {
        ...
        //try to reopen it again... maybe it works..
        fd = open(device, O_RDWR | O_NOCTTY | O_NDELAY | O_SYNC);
        if (fd < 0) {
            fprintf(stderr, "fatal failure, the device could be opened for the 2X\n");
        }
        exit(-1);
    }
    fprintf(stderr, " [OK]\n");
    ...
    for (;;) {
        ...
        switch (mode) {
            case MODE_NODE_TX:
                if (init_request == 0) {
                    ...
                    sleep(1);
                    for (l = 0; CMD_NODE_TX[l] != '\0'; l++) {

                        int nw = write(fd, &CMD_NODE_TX[l], 1);
        ...
        if (exit_req_loop) {
            printf("node started transmission\n");
            break;
        }*/
